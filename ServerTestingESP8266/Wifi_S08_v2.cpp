...
bool ESP8266::startserver(String netName, String password){
	disableTimer();
	delay(500);
	bool ok = true;
	emptyRx();
  Serial.print(AT_CWSAP_SET);
  wifiSerial.print(AT_CWSAP_SET);
	wifiSerial.println('\"' + netName + '\"' + ',' + '\"' + password + '\"' + ',' + "1" + ',' + "4");
	ok = ok && waitForTarget(OK, CWMODE_TIMEOUT);
	if (serialYes) {
		if (ok) {
			Serial.println("CWSAP_SET");
		} else {
			Serial.println("CWSAP SET ERROR");
		}
		Serial.flush();
	}
	ok = true;
	...
	if (serialYes) {
		if (ok) {
			...
			Serial.println("CIPMUX SET ERROR");
		}
		Serial.flush();
	}
	ok = true;
	...
	if (serialYes) {
		if (ok) {
			...
			Serial.println("CIPSERVER SET ERROR");
		}
		Serial.flush();
	}
	enableTimer();
